...
# class EasyVectorizeMeta(type):
#     def __new__(cls, name:str, bases:tuple, dct:dict):
#         dtype = dct.get('dtype', object)
#         array = dct.get('array', np.asarray)

#         class Vectorized(List[dtype]):
#             def _vectorized_attribute(self, attr):
#                 return array([getattr(element, attr) for element in self])

#             def _vectorized_method(self, method):
#                 def _vec_method(*args, **kwargs):
#                     return array([getattr(element, method)(*args, **kwargs) for element in self])
#                 return _vec_method
...
#                 if type(value) != dtype:
#                     raise ValueError("output type is different from", dtype)
#                 else:
#                     return value

#             def __setitem__(self, key, value):
#                 if type(value) != dtype:
#                     raise ValueError("value type is different from", dtype)
#                 else:
...
#                 except:
#                     try:
#                         self[key] = dtype(value)
#                     except:
#                         raise ValueError("Cannot convert value to", dtype)

#         return super().__new__(cls, name, bases, {'Vectorized': Vectorized, **dct})

# class easyvectorize(metaclass=EasyVectorizeMeta):
#     dtype = object
#     array = np.asarray
